...
},
{
  type:'input',
  name:'target',
  message:'[>] Tulis Username Target (rasah nggawe "@") [ENTER]:',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
},
{
  type:'input',
  name:'text',
  message:'[>] Tulis Komenanmu (pisahno nggawe tanda "|") [ENTER]:',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
...
const CommentAndLike = async function(session, accountId, text){
  ...
  if (result.length > 0) {
    ...
    const printFollow = Follow ? chalk`{green SUKSES REK}` : chalk`{red FOLLOW}`;
    const printComment = Comment ? chalk`{green SUKSES REK}` : chalk`{red COMMENT}`;
    const printLike = Like ? chalk`{green SUKSES REK}` : chalk`{red LIKE}`;
  return chalk`[Follow: ${printFollow}] [Like: ${printLike}] [Comment: ${printComment} ({cyan ${text}})]`;
  }
  return chalk`{bold.cyan TIMELINE KOSONG (SKIP WAE)}`
};
...
const Excute = async function(User, TargetUsername, Text, Sleep){
  try {
    ...
    console.log(chalk`{green  [!] Alhamdulillaah iso login,}{yellow [?] Iki Info ID & Followers Targetmu . . .}`)
    ...
    console.log(chalk`{cyan  [?] Njajal Follow, Comment, and Like Followers Targetmu . . . \n}`)
    ...
    do {
      ...
      TargetResult = _.chunk(TargetResult,6);
      for (let i = 0; i < TargetResult.length; i++) {
        ...
        await Promise.all(TargetResult[i].map(async(akun) => {
          if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
            ...
            console.log(chalk`[{magenta ${timeNow}}] {bold.yellow [SKIP WAE]}${akun.params.username} => PRIVATE OR WES MBOK FOLLOW`)
          }
        }));
